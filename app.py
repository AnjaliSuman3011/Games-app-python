...
class Game:
    def __init__(self, player1, player2):
        self.id = str(uuid.uuid4())  # Unique game ID
        self.board = self.create_board()  # Initialize the game board
        self.players = {player1: 'white', player2: 'black'}  # Assign players to colors
        self.turn = 'white'  # White starts the game
        self.moves = []  # List to track moves
    ...
    def make_move(self, move):
        ...
        self.moves.append(move)  # Record the move

@app.route('/register', methods=['POST'])
def register():
    data = request.json
    user_id = str(uuid.uuid4())  # Generate a unique user ID
    users[user_id] = data['username']  # Store the username
...
def create_game():
    ...
    game = Game(player1, player2)  # Create a new game
    games[game.id] = game  # Store the game in memory
...
def on_join(data):
    ...
    join_room(game_id)  # Add the client to the game's room
    emit('update', {"game_id": game_id, "board": games[game_id].board, "turn": games[game_id].turn}, room=game_id)

if __name__ == '__main__':
    socketio.run(app, debug=True)  # Run the app with debug mode enabled
